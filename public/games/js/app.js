(function (global) {
  ...
  let current=null,currentMeta=null,currentScore=0,currentRecord=0,recordBroken=false,paused=false,menuOpen=false,currentDashboard=null;
  ...
  function leaderboardHtml(data){
    ...
    return `<div class="mini-leaderboard"><b>Ranking global · Top 3</b><ol>${data.leaderboard.map((row,index)=>`<li><span>${index+1}. @${esc(row.alias)}</span><strong>${formatNumber(row.score)}</strong></li>`).join('')}</ol></div>`;
  }
  function card(meta){
    const data=gameData(meta.id);
    const footer=meta.multiplayer
      ? `<div class="record-row multiplayer-row"><span>Modo</span><strong>2 jugadores · local</strong></div>`
      : `${leaderboardHtml(data)}<div class="record-row personal-rank"><span>Récord personal${data?.personalRank?` · Puesto #${data.personalRank}`:''}</span><strong>${data?.personalBest?formatNumber(data.personalBest):'—'}</strong></div>`;
  ...
  async function renderHub(){
    ...
    try{currentDashboard=await DG.serverSession.dashboard();}
      ...
      currentDashboard={profile:{alias:'Jugador Denty'},visit:{active:false,rewardGamesCount:0,rewardPotentialCents:0,maxRewardCents:500,voucher:null},games:games.map(g=>({id:g.id,leaderboard:[],personalBest:null,personalRank:null}))};
    }
    const profile=currentDashboard.profile||{alias:'Jugador Denty'};
    app.innerHTML=`<main class="app-frame">${brand(`<span class="profile-chip static"><span class="profile-dot">${esc((profile.alias||'J').slice(0,1).toUpperCase())}</span><span>@${esc(profile.alias||'Jugador Denty')}</span></span>`)}<section class="hero"><p class="eyebrow">Denty Paciente</p><h1>Juega mientras esperas.</h1><p>Rankings entre pacientes de la clínica y tus mejores marcas, siempre dentro de Denty.</p></section>${rewardCard(currentDashboard.visit)}<div class="section-heading"><h2>Juegos</h2><span>Rankings y récords</span></div><section class="games-grid">${games.map(card).join('')}</section><p class="footer-note">Los rankings muestran únicamente alias. Tu nombre clínico nunca aparece en Denty Games.</p></main><div class="overlay" id="overlay" hidden></div>`;
  ...
  async function finishGame(score,subtitle){
    ...
    document.getElementById('finishHome').addEventListener('click',()=>void renderHub());document.getElementById('finishReplay').addEventListener('click',async()=>{if(!(await beginServerPlay(currentMeta.id)))return;overlay.hidden=true;setPaused(false);recordBroken=false;currentScore=0;current.restart();});
  }

(function (global) {
  ...
  function createGame(){
    ...
    function draw(){
      if(!ctx||!state)return;ctx.clearRect(0,0,W,H);ctx.fillStyle=global.DentyGames.shared.cssVar('--dg-canvas-bg','#f8fafc');ctx.fillRect(0,0,W,H);
      ctx.strokeStyle=global.DentyGames.shared.cssVar('--dg-canvas-grid','rgba(50,75,100,.07)');ctx.lineWidth=1;for(let x=0;x<W;x+=48){ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x,H);ctx.stroke();}
      ctx.fillStyle='#e7f4f3';ctx.fillRect(0,GROUND,W,H-GROUND);ctx.fillStyle='#0b7777';ctx.fillRect(0,GROUND,W,3);
      for(const o of state.obstacles){if(o.type==='gap'){ctx.fillStyle=global.DentyGames.shared.cssVar('--dg-canvas-bg','#f8fafc');ctx.fillRect(o.x,GROUND-2,o.w,H-GROUND+4);continue;}ctx.fillStyle=o.type==='spike'?'#4055a8':o.type==='bar'?'#6750b5':'#dbe7f5';if(o.type==='spike'){ctx.beginPath();ctx.moveTo(o.x,o.y+o.h);ctx.lineTo(o.x+o.w/2,o.y);ctx.lineTo(o.x+o.w,o.y+o.h);ctx.closePath();ctx.fill();}else{global.DentyGames.shared.roundedRect(ctx,o.x,o.y,o.w,o.h,7);ctx.fill();}}

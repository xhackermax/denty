...
describe("state machines", () => {
  it("preserva las transiciones clínicas de cita", () => {
    ...
    expect(
      canTransitionAppointment("ARRIVED", "confirm_waiting_room", "RECEPTION", false),
    ).toBe(true);
    expect(
      canTransitionAppointment("ARRIVED", "confirm_waiting_room", "DENTIST", false),
    ).toBe(false);
    expect(
      canTransitionAppointment("ARRIVED", "confirm_waiting_room", "RECEPTION", true),
    ).toBe(false);
  });
  ...
  it("exige receta validada, paciente, prescriptor y medicamento", () => {
    ...
    expect(
      canIssuePrescription({ state: "READY", patientId: "p1", medicationCount: 1 }),
    ).toBe(false);
  });

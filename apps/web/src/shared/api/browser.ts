...
import {
  DENTY_EVENTS_PATH,
  openDentyEventStream,
  type DentyEventStreamOptions,
} from "./events";
...
export function getBrowserApi() {
  ...
  browserApi ??= createDentyApi(
    new ApiClient({ baseUrl: DENTY_BROWSER_API_BASE_URL }),
  );
  return browserApi;
}

export function openBrowserDentyEventStream(
  options: Omit<DentyEventStreamOptions, "url">,
) {

...
export function createPortalResource(client: ApiClient) {
  return {
    appointments: {
      availability: (
        patientId: string,
        query: z.input<typeof patientAvailabilityQuerySchema>,
      ) => {
    ...
    invitations: {
      create: (
        patientId: string,
        payload: z.input<typeof createPatientInvitationSchema>,
      ) =>
        client.mutation(
    ...
    projection: (patientId: string) =>
      client.request(
        `/api/patient/${encodeId(patientId)}/projection`,
        patientProjectionSchema,
      ),
    ...
    checkIn: (appointmentId: string) =>
      client.mutation(
        `/api/patient/check-in/${encodeId(appointmentId)}`,
        appointmentSchema,
        {},
      ),
    respondToBudget: (
      budgetId: string,
      payload: z.input<typeof budgetResponseSchema>,
    ) =>
      client.mutation(

  ...
  const patterns = [
    ...
    new RegExp(
      "(?:busca|buscar|abre|abrir|encuentra|ver|selecciona|carga)\\s+" +
        `(?:la\\s+)?(?:ficha\\s+de\\s+|paciente\\s+|a\\s+)?${name}`,
      "i",
    ),
    new RegExp(
...
function readback(actions: readonly LocalVoiceAction[], ambiguities: readonly string[]): string {
  ...
  return parts.length
    ? `Voy a ${uniq(parts).join("; ")}.`
    : "No he detectado una acción concreta.";
}
...
export function planLocalVoiceCommand(
  ...
  const text = normalize(raw);
  const creatingPatient = /^(?:crea|crear|nuevo|nueva|alta|registra)\b/.test(text) &&
    /\b(?:paciente|ficha)\b/.test(text);
  ...
  const schedulesAppointment =
    /\b(?:pon|agenda|cita|programa|programar|citar|dame\s+cita)\b/.test(text);
  ...
  const budgetIntent = /(?:prepara|haz|genera|crear|crea).*presupuesto/.test(text) ||

...
export function CommunicationsModule() {
  ...
  return (
    <>
      <Alert color="blue" title="Canales externos desacoplados" mb="md">
        La demo conserva categorías, consentimiento y estados. WhatsApp, SMS y email
        reales se enviarán por adaptadores de backend.
      </Alert>
      ...
      <section className={styles.section}>
        <div className={styles.sectionHeader}>
          ...
          <Button size="xs" onClick={() => setOpened(true)}>Nueva comunicación</Button>
        </div>
          ...
          {rows.map((row) => (
            <div className={styles.row} key={row.id}>
              <div className={styles.rowMain}>
                <span className={styles.rowTitle}>{row.patientName} · {row.subject}</span>
                <span className={styles.rowMeta}>{row.id} · {row.channel} · {row.category}</span>
              </div>
              <div className={styles.rowActions}>
                <Badge color={statusColor(row.status)} variant="light">{row.status}</Badge>
                {row.status === "QUEUED" ? (
      ...
      <Modal opened={opened} onClose={() => setOpened(false)} title="Nueva comunicación">
        <Stack>
          <Select
            label="Paciente"
            value={patientId}
          ...
          {category === "MARKETING" ? (
            <Switch
              ...
              label="Consentimiento de marketing vigente"
            />
          ) : null}

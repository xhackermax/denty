  ...
  return (
    <section className={styles.section}>
      <Group justify="space-between" align="flex-start">
        <div>
          <Title order={3}>Historial del odontograma</Title>
        ...
        <Badge variant="light">
          {demoMode
            ? "Demo"
            : `v${snapshotsQuery.data?.currentVersion ?? "?"}`}
        </Badge>
      </Group>

      {demoMode ? (
        <Alert mt="lg" color="blue" title="Historial de demostración">
          El modo demo conserva solo el historial local del editor. Los snapshots del
          servidor no se simulan.
        </Alert>
      ) : (
        <>
          ...
          {createMutation.isError ? (
            <Alert mt="lg" color="red" title="No se pudo guardar el snapshot">
              El historial remoto no se ha sustituido por una copia local.
            </Alert>
          ) : null}

          {snapshotsQuery.isError ? (
            <Alert mt="lg" color="red" title="No se pudo cargar el historial">

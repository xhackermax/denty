...
import type { ClinicalPlan, ClinicalSyncState, ClinicalWorkflow } from "@/shared/api";
...
function EndodonticsPanel({ patientId, demoMode, workflow }: ClinicalPanelProps) {
  ...
  const [pulpal, setPulpal] = useState<PulpalDiagnosis>("Pulpitis irreversible sintomática");
  const [apical, setApical] = useState<ApicalDiagnosis>("Periodontitis apical sintomática");
  ...
  const latestAssessment = workflow?.endodonticAssessments.find((item) => item.tooth === tooth);
  ...
  return (
    <section className={styles.section}>
      ...
      {!demoMode && latestAssessment ? (
        <Alert mt="lg" color="green" title="Guardado">
      ...
      {assessmentMutation.isError || planMutation.isError ? (
        <Alert mt="lg" color="red" title="No se guardó">
...
function PeriodontalPanel({ patientId, demoMode, workflow }: ClinicalPanelProps) {
  ...
  return (
    <section className={styles.section}>
      ...
      {!demoMode && latestExam ? (
        <Alert mt="lg" color="green" title="Examen guardado">
          {latestExam.stage ?? "Stage —"} · {latestExam.grade ?? "Grade —"} ·{" "}
          {latestExam.extent ?? "Extensión —"}
        </Alert>
      ) : null}
      {examMutation.isError ? (
        <Alert mt="lg" color="red" title="No se guardó el examen">
...
function DemoPlanBudgetPanel() {
  ...
  return (
    <div className={styles.gridTwo}>
      ...
      <section className={styles.section}>
        <Title order={3}>Presupuesto · demo</Title>
        <div className={styles.rowList}>
          {PLAN_ITEMS.map((item) => (
            <div className={styles.row} key={item.id}>
              <Checkbox
...
function ServerPlanBudgetPanel({ patientId, plan, sync }: ServerPlanBudgetPanelProps) {
  ...
  return (
    <div className={styles.gridTwo}>
      <section className={styles.section}>
        ...
        {planSync.isError ? (
          <Alert mt="lg" color="red" title="No se sincronizó el plan">
      ...
      <section className={styles.section}>
        <Group justify="space-between" align="flex-start">
          <div>
            <Title order={3}>Presupuesto</Title>
            <Text c="dimmed" size="sm" mt="xs">
        ...
        {sync.budget ? (
          <div className={styles.metric}>
            ...
            <strong className={styles.metricValue}>{formatEUR(sync.budget.totalCents)}</strong>
          </div>
        ) : (
        ...
        {budgetSync.isError ? (
          <Alert mt="lg" color="red" title="No se sincronizó el presupuesto">
...
export function ClinicalWorkspace({ patientId, demoMode }: ClinicalWorkspaceProps) {
  ...
  if (!demoMode && serverError) {
    return (
      <Alert color="red" title="Error al cargar clínica">
  ...
  if (!demoMode && (!workflowQuery.data || !planQuery.data || !syncQuery.data)) {
    return (
      <Alert color="blue" title="Cargando clínica">
  ...
  return (
    <div className={styles.grid}>
      ...
      />
      {tab === "endo" ? (
        <EndodonticsPanel patientId={patientId} demoMode={demoMode} workflow={workflowQuery.data} />
      ) : null}
      {tab === "perio" ? (

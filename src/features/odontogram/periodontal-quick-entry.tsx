...
import {
  useOdontogramQuery,
  useRecordPeriodontalMeasurementMutation,
...
export function PeriodontalQuickEntry({
  patientId,
  demoMode,
}: PeriodontalQuickEntryProps) {
  ...
  return (
    <section className={styles.section}>
      <Group justify="space-between" align="flex-start">
        <div>
          <Title order={3}>Registro periodontal rápido</Title>
      ...
      {mutation.isError ? (
        <Alert mt="lg" color="red" title="No se pudo registrar la medición">
          El odontograma conserva la versión previa; no se ha escrito un valor local.
        </Alert>
      ) : null}

      {!demoMode && latest ? (
        <Alert mt="lg" color="green" title="Última medición disponible">
          Diente {latest.tooth} · {latest.site} · PD{" "}
          {latest.probingDepth ?? "—"} mm · recesión {latest.recession ?? "—"} mm
        </Alert>
      ) : null}
    </section>
  );

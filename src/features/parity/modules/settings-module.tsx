...
type SettingsTab = "sessions" | "agenda" | "backups" | "privacy" | "prescription" | "marketing";
...
export function SettingsModule() {
  ...
  const savePlanVisitGapDays = (value: number | string) => {
    const next = normalizePlanVisitGapDays(value);
    setPlanVisitGapDays(next);
    window.localStorage.setItem(
      DEMO_SCHEDULING_STORAGE_KEY,
      JSON.stringify({ defaultPlanVisitGapDays: next }),
    );
  };
  ...
  return (
    <div className={styles.grid}>
      ...
      {tab === "agenda" ? (
        <section className={styles.section}>
          ...
          <Text c="dimmed" size="sm" mt="xs">
            Preferencias al crear series desde un plan.
          </Text>
          <NumberInput
            mt="lg"
            label="Intervalo entre citas"
            description="Días entre visitas generadas desde un plan."
            min={0}
            max={180}
            step={1}
            value={planVisitGapDays}
            onChange={savePlanVisitGapDays}
          />
          <Alert color="blue" mt="md">
            Las citas existentes no cambian. En pruebas el valor inicial es 7 días.
          </Alert>
        </section>
      ) : null}
      ...
      {tab === "backups" ? (
        <section className={styles.section}>
          <div className={styles.sectionHeader}>
            ...
            <Button size="xs" onClick={createBackup}>
              Crear copia
            </Button>
          </div>
          <Alert color="blue" mb="md">
            La restauración productiva se ejecutará en backend/CLI sobre una copia validada antes de
      ...
      {tab === "privacy" ? (
        <section className={styles.section}>
          <Title order={3}>Privacidad</Title>
          <Text c="dimmed" size="sm" mt="xs">
          ...
          <Stack mt="md">
            <Group grow>
              ...
              <Select
                label="Solicitud"
                value={privacyType}
                onChange={(value) => setPrivacyType((value as PrivacyRow["type"]) ?? "ACCESS")}
            ...
            <Group justify="flex-end">
              <Button size="xs" onClick={createPrivacyRequest}>
                Registrar solicitud
              </Button>
            </Group>
          </Stack>
        ...
        <section className={styles.section}>
          ...
          <Stack>
            ...
            <Switch
              ...
              label="Recepción crea borradores"
            />
            <Group justify="flex-end">
              <Button size="xs">Guardar configuración</Button>
            </Group>
          </Stack>
        </section>
      ) : null}

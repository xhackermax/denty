...
function conflictsByResource(
  ...
  const sameCabinet = Boolean(moving.cabinetId) && moving.cabinetId === other.cabinetId;
...
export function findConflicts(
  ...
  const blockConflicts = blocks
    .filter((block) => {
      ...
      const cabinetConflict = Boolean(block.cabinetId) && block.cabinetId === moving.cabinetId;
...
export function resizeTo(appointment: AgendaAppointment, minutes: number): AgendaAppointment {
...
export function layoutDay(
  ...
  const sorted = [...appointments]
    ...
    .sort(
      (a, b) =>
        a.start - b.start || a.end - b.end || a.appointment.id.localeCompare(b.appointment.id),
    );
  ...
  for (const node of nodes) {
    columnsByGroup.set(node.group, Math.max(columnsByGroup.get(node.group) ?? 0, node.column + 1));
  }
